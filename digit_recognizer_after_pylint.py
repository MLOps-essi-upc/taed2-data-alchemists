...
# MLflow package for recording information of the run
import mlflow
...
def train(n_epochs, loaders, model, optimizer, criterion, use_cuda, save_path):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        print("Epoch: " f"{epoch}" "\tTraining Loss: " f"{train_loss}" "\tValidation Loss: " f"{valid_loss}")

        ## Save the model if validation loss has decreased
        if valid_loss < valid_loss_min:
            print("Validation loss decreased (" f"{valid_loss_min}" "-->" f"{valid_loss}"").  Saving model ...")
...
# Train the model
model = train(1, loaders, model, optimizer,
...
def test(loaders, model, criterion, use_cuda):
    ...
    print("\nTest Accuracy: " f"{100. * correct / total}"" (" f"{correct}""/"f"{total}")
